...
N_PIXEL = 415
PIXEL_PIXEL_DISTANCE = 4200.0 # micron
LIMIT_RADIUS = 1.0e5 * 0.5 # 4" = 100mm
LIMIT_RADIUS_INTERNAL = 60000.0*0.5
...
for (npr, fst_x, fst_y) in zip(NUMBER_PER_ROW, FIRST_ELEMENT_X_HEX, FIRST_ELEMENT_Y_HEX):
    ...
    for npr_i in range(npr):
        ...
        table.append([PIXEL_ORDER[px_i], x, y])
        px_i += 1
...
# make csv file
csv = open("pixel_positions_{:d}.csv".format(N_PIXEL),"w")
csv.write("index,x[mm],y[mm]\n")
for i in range(N_PIXEL):
    csv.write("{:d},{:.6f},{:.6f}\n".format( int(table_sorted[i,0]), table_sorted[i,1]*1e-3, table_sorted[i,2]*1e-3))
...
for i in range(N_PIXEL):
    plt.text(table_sorted[i,1]*1e-3, table_sorted[i,2]*1e-3, str(int(table_sorted[i,0])), ha="center", va="center")
#plt.plot(table_sorted[:,1]*1e-3, table_sorted[:,2]*1e-3, '-', c='gray', alpha=0.4)

plt.xlim([-50.400, 50.400])
plt.ylim([-50.400, 50.400])
ax = plt.gca()
ax.set_aspect('equal')
plt.show();
