...
def squareTriangleLattice(hside, pitch, vside=None, element_dimension=0.0, rotation=0):
    '''
    This function generates the coordinates of a square lattice inside a 
    circle of a given radius
    ...
    ----------
    hside : float
        The horizontal side of the square in microns.
    vside : float
        The vertical side of the square in microns. Default is None
    pitch : float
        The unit cell length of the lattice in microns.
    element_dimension : float
        The dimension of a node of the lattice, it coincides with the absorber 
        side in microns. Default is 0.0
    rotation : int, optional
        This parameter can be 1, 0 or -1. If 1 all the even rows (starting from
        the lower one) will be rotated by 180 degrees, if -1 all the odd rows 
        will be rotated by 180 degrees, if 0 no rotation will be applied.
    ...
    #maximum_number_diameter_x = int(radius // x_step)*2 - 1
    #maximum_number_diameter_y = int(radius // y_step)*2 - 1
    x_min = -int(hside*0.5 // x_step)*x_step#-(radius // x_step)*x_step
    y_min = -int(vside*0.5 // y_step)*y_step#-(radius // y_step)*y_step

    for i,y_i in enumerate(np.arange(y_min-y_step, -y_min+1, step=y_step)):
        for j,x_j in enumerate(np.arange(x_min-x_step, -x_min+1, step=x_step)):
            if -vside*0.5+element_dimension*0.5 <= y_i <= vside*0.5-element_dimension*0.5 and -hside*0.5+element_dimension*0.5 <= x_j+((i+1)%2)*x_step*0.5 <= hside*0.5-element_dimension*0.5:
    ...
    fig = plt.figure()
    plt.xlim([-1.2*hside, 1.2*hside])
    plt.ylim([-1.2*vside, 1.2*vside])
    ax0 = plt.gca()
    ax0.set_xlabel('x position [microns]')
    ax0.set_ylabel('y position [microns]')
    ax0.set_aspect('equal')
